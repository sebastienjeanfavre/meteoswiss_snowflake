-- ============================================================================
-- Bronze Table: Realtime Now Weather Measurements (10-minute intervals)
-- ============================================================================
-- Realtime 10-minute weather measurements from MeteoSwiss stations
--
-- Data coverage: Yesterday 12:00 UTC → Now (current time)
-- Granularity: 10-minute intervals
-- Update frequency: Every 10 minutes (automated)
...
CREATE TABLE IF NOT EXISTS bronze.t_weather_measurements_10min_now (
    -- Metadata
...
COMMENT ON TABLE bronze.t_weather_measurements_10min_now IS
    'Realtime 10-minute weather measurements from MeteoSwiss stations (yesterday 12:00 UTC to now, updated every 10 minutes)';
COMMENT ON COLUMN bronze.t_weather_measurements_10min_now.station_abbr IS 'Station abbreviation code';
COMMENT ON COLUMN bronze.t_weather_measurements_10min_now.reference_timestamp IS 'Measurement timestamp (10-minute intervals)';
COMMENT ON COLUMN bronze.t_weather_measurements_10min_now.gre000z0 IS 'Global solar radiation in W/m²';
COMMENT ON COLUMN bronze.t_weather_measurements_10min_now.file_name IS 'Source CSV filename for audit trail';
