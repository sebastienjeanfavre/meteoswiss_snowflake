...
-- Create the task
CREATE OR REPLACE TASK common.task_bronze_load_weather_stations
    WAREHOUSE = METEOSWISS_WH
...
-- SHOW TASKS LIKE 'task_bronze_load_weather_stations' IN SCHEMA common;

-- To view task run history:
-- SELECT *
-- FROM TABLE(INFORMATION_SCHEMA.TASK_HISTORY(
--     SCHEDULED_TIME_RANGE_START => DATEADD('day', -30, CURRENT_TIMESTAMP()),
--     TASK_NAME => 'TASK_BRONZE_LOAD_WEATHER_STATIONS'
-- ))
-- ORDER BY SCHEDULED_TIME DESC;

-- To manually execute the task (for testing):
-- EXECUTE TASK common.task_bronze_load_weather_stations;
